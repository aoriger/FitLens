    ...
    case HCI_VENDOR_SPECIFIC_DEBUG_EVT_CODE:
      {
        ...
        switch (blecore_evt->ecode)
        {
          ...
          {
            /* Cast the data to a Notification Event structure */
            aci_gatt_notification_event_rp0 *notification_data = (aci_gatt_notification_event_rp0*)blecore_evt->data;
            
            /* Check if this notification is from the Attribute Handle we care about. 
               (For now we assume it is, since we only connect to one thing) */
             
            uint8_t *data = notification_data->Attribute_Value;
            uint8_t flags = data[0];
            uint16_t heart_rate = 0;

            /* Parse the Flags (Bit 0 tells us format) */
            if ((flags & 0x01) == 0) 
            {
               /* Bit 0 is 0: Heart Rate is UINT8 (1 byte) */
               heart_rate = data[1];
            }
            else 
            {
               /* Bit 0 is 1: Heart Rate is UINT16 (2 bytes) */
...

              {
                ...
                while(k < event_data_size)
                {
                  ...
                  switch (adtype)
                  {
                    ...
                    {
                      uint8_t i = 0;
                      while ((i + 2) <= (adlength - 1)){
                        uint16_t uuid = (adv_report_data[k + 2 + i]) | (adv_report_data[k + 3 + i] << 8);
